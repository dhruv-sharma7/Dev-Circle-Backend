...
router.post("/", authenticate, async (request, response) => {
  try {
    ...
    let newPost = {
      ...
      image: request.body.image,
      name: user.name,
      avatar: user.avatar ? user.avatar : "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTTOkHm3_mPQ5PPRvGtU6Si7FJg8DVDtZ47rw&usqp=CAU", // Add a default value if avatar is missing or undefined
    };
    let post = new Post(newPost);
    post = await User.populate(post, { path: "user.avatar" });
    post = await post.save();
