...
router.post("/register", async (request, response) => {
  try {
    ...
    const token = jwt.sign({ user: { id: user.id, name: user.name } }, process.env.JWT_SECRET_KEY);

    response.status(200).json({ msg: "Registration is successful", token });
  } catch (error) {
    console.error(error);
    response.status(500).json({ errors: [{ msg: error.message }] });
  }
...
router.post("/login", async (request, response) => {
  try {
    ...
    response.status(200).json({ msg: "Login is successful", token });
  } catch (error) {
    console.error(error);
    response.status(500).json({ errors: [{ msg: error.message }] });
  }
